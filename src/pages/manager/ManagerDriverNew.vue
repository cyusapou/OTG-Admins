  ...
  >
    <div v-if="created" class="success-card">
      ...
      <div class="success-actions">
        <router-link to="/manager/drivers" class="btn-secondary">Back to Drivers</router-link>
        <button class="btn-primary" @click="resetForm">Add Another</button>
      </div>
    </div>

    <form v-else class="form-card" @submit.prevent="handleSubmit">
      <router-link to="/manager/drivers" class="back-link">
      ...
      <div class="form-grid">
        ...
        <div class="field">
          ...
          <input v-model="form.licenseNumber" type="text" required placeholder="DL-XXXXX" />
        </div>
        <div class="field">
        ...
        <div class="field">
          ...
          <select v-model="form.busId">
            <option value="">No bus assigned</option>
            <option v-for="bus in availableBuses" :key="bus.id" :value="bus.id">
              {{ bus.plateNumber }} — {{ bus.model || 'Unknown' }} ({{ bus.capacity }} seats)
            </option>
          </select>
        </div>
        <div class="field">
          <label>Username</label>
          <input v-model="form.username" type="text" required placeholder="john.doe" />
        </div>
        <div class="field">
      ...
      <button type="submit" class="btn-primary btn-submit" :disabled="submitting">
        ...
        <span v-else><i class="fas fa-plus"></i> Create Driver</span>
      </button>
    </form>
  </PortalLayout>
</template>
...
async function loadBuses() {
  try {
    ...
    const buses = await busService.getAll({ depotId })
    availableBuses.value = buses.filter(b => !b.driverId && b.status !== 'maintenance')
  } catch {}
}
